...
2. **Source real assets first, generate as fallback.** Visual sourcing priority (try in order):
   1. **Real product screenshots / UI** — if the speaker names a real tool ("Codex", "Stripe Dashboard", "Cursor", "Linear"), find the actual interface. Use `WebSearch` + `WebFetch` to locate official press kits, product pages, or screenshots, then `curl -L` to download. Save under `<workdir>/broll/<index>.<ext>`.
...
4r. **"Cloud" → "Codex" — automatic, no exceptions.** Whisper consistently mishears "Codex" as "Cloud" on this channel's audio. Every video processed by this skill is about Codex / Codex, so [scripts/transcribe.py](scripts/transcribe.py) substitutes `cloud → Codex` (capitalization-preserving) in `words.json` before downstream tools read it. This means: speech_anchor matching uses "Codex", list-item keyword matching uses "Codex", and any rendered captions/quotes/labels you author MUST use "Codex" not "Cloud". When auditing existing plans, grep for "cloud" and fix every reference (anchor, value, caption, callout_*, list items, vs_split items, top/bottom_label, quote_text). The substitution rule lives in `_normalize_brand_terms()` in transcribe.py — extend that function if a new branded term ever needs the same treatment.
...
```json
{
  ...
  "bottom_items": ["One Codex routine", "40 lines of Markdown", "Runs forever"],
  "winner": "bottom"
}
```
...
- The "stack" enumeration ("ChatGPT, Codex, Grok") — one item per word, timed to the speaker.
...
```json
{
  ...
  "start_sec": 5.8,
  "end_sec": 12.2,
  "speech_anchor": "they used chatgpt Codex and grok",
  "vertical": 0.72,
  "items": [
    { "text": "ChatGPT",                       "appear_sec": 6.2 },
    { "text": "{Codex}",                       "appear_sec": 7.0 },
...
{"heading":"AGENTS.md < 1k tokens","description":"...","appear_sec":20.0},
...
4bp. **💻 `claude_code_terminal` — tiny Mac-style terminal frame with typewriter lines, themed for Codex.** Codified May 23 2026 ("we don't really do that much in the terminal ourselves, but Codex lives there"). For moments referencing the CLI tool — `/compact`, `/clear`, `AGENTS.md`, MCP config — show a small terminal window with the relevant `Codex>` prompts + Codex responses typing in. [ClaudeCodeTerminal.tsx](remotion/src/templates/ClaudeCodeTerminal.tsx) kind `claude_code_terminal`. Partial overlay (speaker stays visible). `lines: [{type, text, appear_sec, cps?}]` — `type` is `user` (lime `Codex>` prompt), `Codex` (white output), or `system` (subdued info `#`). Use it instead of raw code or a screenshot when the message is "this is what happens INSIDE Codex."
...
4br. **🟧 When the speaker says "Codex" — ALWAYS show the Anthropic logo.** Codified May 23 2026 ("a lot of videos don't show Codex's logo!"). Codex is the channel's most-mentioned brand; if a video says "Codex" and you can't see Anthropic anywhere on screen, the visual is incomplete. Rule:

- **Every scene that names Codex must show `anthropic.png` somewhere on screen** — usually as a `tool_logo_burst` item at or near the FIRST `"Codex"` word in `words.json`. Equivalent visuals also satisfy the rule: a `portrait_burst` of Dario Amodei (he IS Anthropic), or a `claude_code_terminal` (the title literally reads "Codex").
- When inserting, find the first 1.5s+ gap near the first "Codex" word and put a single-item `tool_logo_burst` with `image_path: "anthropic.png"`, `label: "Codex"`, `accent: true` (lime ring). If no clean gap exists, *augment* an adjacent `tool_logo_burst` to include the Codex logo, or `accent` the existing Codex reference.
- This extends rule 4bm's "always when a real business is named" with a Codex-specific priority — Anthropic is the channel's house brand.
...
4bk. **🪧 `hook_title.title` MUST be short (≤16 chars / 1-2 lines). Stats and details go in the KICKER.** Codified May 23 2026 after scene-10's hook used `title="67% RATE Codex CLEANER"` (22 chars) which the auto-sized hook font wrapped to 3 dominant lines that swallowed the whole frame. The `title` is the hero word — it should be a SHORT punchline (3-16 chars, 1-2 lines max at 9:16). The `kicker` is the smaller line where supporting stats/context live. Examples:
- ✅ `kicker:"65% CODEX · 67% Codex", title:"BOTH TRUE."` — stats in kicker, conclusion in title
- ✅ `kicker:"I PAY $200/MO FOR Codex", title:"I NEVER HIT\nTHE LIMIT"` — long context kicker, short punchline title
- ❌ `kicker:"65% PREFER CODEX", title:"67% RATE Codex CLEANER"` — title too long, wraps to 3 lines
...
### Stage 2 — Plan b-roll (Codex does this)
...
- **Anchor on concrete things**: numbers ("$1,500"), named tools ("Stripe", "Codex"), metaphors ("guardrails", "the funnel"), enumerated lists
...
[
  {
    ...
    "end_sec": 11.5,
    "kind": "static",
    "reason": "Speaker names 'Codex' — show the actual terminal interface so viewers see what we're talking about",
    "source": "real-screenshot",
    "image_path": "broll/1-Codex.png",
    "search_query": "Codex CLI terminal screenshot site:anthropic.com"
  },
  {
    "start_sec": 22.0,
    "end_sec": 30.0,
...
`cutplan.py`, `selector.py`, `sentences.py`, `Captions.tsx` are kept in the repo for reference but are NOT part of the v3 pipeline. The new flow only needs: `transcribe.py` → b-roll planning (Codex) → image gen → `zoom_plan.py` → `render.sh` → `verify.py`.
