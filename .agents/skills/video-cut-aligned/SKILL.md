...
```
`recut.py` is the SAME code the media agent's `/recut` runs, so your cuts and Luuk's self-serve edits are byte-identical. It prints `{"ok":..,"duration":..,"verify_ok":..}`. Use this for re-cuts; use the explicit steps below when you want to inspect the intermediate `cut_raw.mp4`. **It also applies the BuildLoop cinematic grade by default** — the same b-roll Rec709 LUT (`~/.Codex/skills/broll-ingest/luts/buildloop-FINAL.cube`); pass `--no-grade` to skip. So every exported short is graded (one-time backfill of existing shorts: `ffmpeg -i SHORT.mp4 -vf "lut3d=file='<LUT>',format=yuv420p" -c:v libx264 -crf 18 -preset medium -c:a copy -movflags +faststart OUT.mp4`).
...
1. **Read the queued notes** via the Supabase MCP (project `xchadavewdhgirwpkkzk`):
   ...
   ```
   (`ready` = he hit "Send to Codex". If nothing's `ready`, also check `status='open'` — he may have skipped that button.)
...
# (talking-head footage: keep the LUT — add lut3d=file='$HOME/.Codex/skills/broll-ingest/luts/buildloop-FINAL.cube' before format=yuv420p)
